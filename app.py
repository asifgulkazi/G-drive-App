                                    ...
                                    if row.Action == 'Copy':
                                        ...
                                        if not capabilities_dict.get('canCopy', False): log_entry['Status'] = 'Skipped (Copy restricted)'
                                            ...
                                            try:
                                                ...
                                                log_entry.update({'Status': 'Copied to Drive', 'New Name': copied_file['name'], 'Path': dest_path, 'Size (MB)': float(f"{int(copied_file.get('size', 0)) / (1024*1024):.2f}") if copied_file.get('size') else 'N/A', 'Link': copied_file.get('webViewLink'), 'Owner': user_info['user_name']})
