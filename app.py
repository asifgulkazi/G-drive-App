...
def get_authorized_users():
  ...
  creds_dict = st.secrets["gspread_service_account"]
        ...
        st.error(f"FATAL: Could not read authorized users list.
Error: {e}")
        return None
...
text-align: center; padding: 40px; background-color: #f4f4f4;">
            <div style="max-width: 600px;
margin: auto; background-color: white; border: 1px solid #ddd; border-radius: 10px;
padding: 20px;">
                <h2>Access Request for {APP_NAME}</h2>
                <p>A new user has requested access to the application.</p>
                <p style="font-size: 1.2em;
margin: 20px 0;"><b>User's Email:</b> {user_email}</p>
                <p>To grant access, click the button below. This will run a script to add the user and notify them.</p>
                <a href="{approval_link}" style="background-color: #28a745;
color: white; padding: 14px 25px; text-align: center; text-decoration: none; display: inline-block; border-radius: 8px; font-size: 16px;
margin-top: 20px;">Grant Access</a>
                <p style="font-size: 0.8em;
color: #777; margin-top: 30px;">If the button does not work, you can copy this link into your browser:<br><a href="{approval_link}">{approval_link}</a></p>
            </div></body></html>"""
        msg = EmailMessage()
...
Exception as e:
        st.error(f"Could not send request email.
...
def get_file_icon(item):
    ...
    mime_type = item.get('effective_mime', item.get('mimeType', ''));
icon_map = {'application/pdf': '📕','application/vnd.google-apps.document': '📝','application/vnd.openxmlformats-officedocument.wordprocessingml.document': '📝','application/vnd.google-apps.spreadsheet': '📊','application/vnd.openxmlformats-officedocument.spreadsheetml.sheet': '📊','application/vnd.google-apps.presentation': '📽️','application/vnd.openxmlformats-officedocument.presentationml.presentation': '📽️','application/zip': '📦','application/x-zip-compressed': '📦'}
...
errors.append(f"Could not fetch largest files: {e}")
...
page_token = response.get('nextPageToken', None)
        ...
        except HttpError as e: errors.append(f"Could not get count for '{friendly_name}': API Error. Skipping.");
continue
...
def list_folder_contents(service, folder_id):
    all_items, total_size = [], 0;
root_details = get_file_details(service, folder_id)
...
page_token = None
        while True:
            ...
            except HttpError as e: st.warning(f"Could not access folder: {e}");
break
            for item in results.get('files', []):
                path = os.path.join(path_prefix, item['name']);
size = int(item.get('size', 0)); total_size += size; all_items.append({**item, 'Path': path})
                if item['mimeType'] == 'application/vnd.google-apps.folder': recurse(s, item['id'], path)
            page_token = results.get('nextPageToken')
            if not page_token: break
    recurse(service, folder_id, root_details['name']);
return all_items, total_size
...
def get_owner_and_all_items_recursive(_service, file_id):
    ...
    def recurse(f_id, path_list):
        ...
        while True:
            try:
                fields_to_get = "nextPageToken, files(id, name, mimeType, webViewLink, capabilities, owners, modifiedTime, size)";
results = _service.files().list(q=f"'{f_id}' in parents and trashed=false", fields=fields_to_get, supportsAllDrives=True, includeItemsFromAllDrives=True, pageSize=200, pageToken=page_token).execute()
                for item in results.get('files', []):
                    current_path = path_list + [item['name']];
item['path'] = os.path.join(*current_path); all_items.append(item)
                    if item['mimeType'] == 'application/vnd.google-apps.folder': recurse(item['id'], current_path)
                page_token = results.get('nextPageToken')
                if not page_token: break
            except HttpError as e: st.warning(f"Could not access subfolder content: {e}");
break
...
def get_and_sort_folder_items(_service, folder_id, current_user_email):
    ...
    while True:
        try:
            fields = "nextPageToken, files(id, name, mimeType, size, webViewLink, modifiedTime, owners, shortcutDetails, capabilities)";
results = _service.files().list(q=f"'{folder_id}' in parents and trashed=false", fields=fields, pageSize=500, pageToken=page_token, supportsAllDrives=True, includeItemsFromAllDrives=True).execute()
            items.extend(results.get('files', []));
page_token = results.get('nextPageToken')
            if not page_token: break
        except Exception as e: st.error(f"Failed to fetch Drive items: {e}");
break
    processed_items = []
    for item in items:
        is_shortcut = item.get('mimeType') == 'application/vnd.google-apps.shortcut';
effective_mime, effective_owners = item.get('mimeType'), item.get('owners')
        if is_shortcut:
        ...
        is_folder = effective_mime 
== 'application/vnd.google-apps.folder'; owner_email = effective_owners[0].get('emailAddress', '') if effective_owners else '';
is_owned_by_me = owner_email == current_user_email
    ...
    processed_items.sort(key=lambda x: (x['is_folder_sort'], x['is_owned_by_me_sort'], x['name_sort']));
return processed_items

def analyze_content(all_items):
    promo_keywords = ['subscribe', 'join', 'channel', 'promo', 'telegram', 'read', 'watch'];
names = [item['name'] for item in all_items]; name_counts = Counter(names);
...
for item in items_list:
        owner_name = item.get('owners', [{}])[0].get('displayName', 'N/A');
mod_time = pd.to_datetime(item.get('modifiedTime')).strftime('%Y-%m-%d %H:%M') if item.get('modifiedTime') else 'N/A'; size_mb = float(f"{int(item.get('size', 0)) / (1024*1024):.2f}") if 'size' in item and item['size'] is not None else 0.0
...
def reset_cleaner_state():
    st.session_state.cleaner_state = 'initial';
st.session_state.cleaner_link = ""; st.session_state.cleaner_root_details = None; st.session_state.cleaner_all_items = []; st.session_state.cleaner_success_log = None; st.session_state.cleaner_skipped_log = None;
st.session_state.cleaner_dest_folder_name = None
...
def run_main_app(service, user_info):
    with st.sidebar:
        ...
        st.write("---")
        st.header("Menu")
        PAGES = ["Dashboard", "File Explorer", "Cloud Copy", 
"Bulk File Cleaner"]
...
st.rerun()
    ...
    if st.session_state.page == "Dashboard":
        # MODIFIED: Removed the generic operation summary from here.
        st.markdown(f"Welcome, **{storage['user_name']}**!
Here's a quick snapshot of your Google Drive.")
...
with st.container(border=True, height=450):
                        ...
                        if drive_stats['recent_files']:
                            recent_files_data = [{"Name": item.get('name', 'N/A'), "Type": "Folder" if item.get('mimeType') == 'application/vnd.google-apps.folder' else "File", "Last Modified": pd.to_datetime(item.get('modifiedTime')).strftime('%Y-%m-%d %H:%M'), "Link": 
item.get('webViewLink', '#')} for item in drive_stats['recent_files']]
...
nav_items.append({'type': 'back', 'name': '⬅️ Back'})
...
st.markdown("""<style>.sticky-header{position:sticky;top:50px;background-color:white;z-index:10;display:flex;flex-direction:row;align-items:center;padding:10px 5px;border-bottom:1px solid #e6e6e6;}.header-col{font-weight:bold;text-align:left;padding:0 4px;color:#262730;}.back-to-top{position:fixed;bottom:20px;right:25px;font-size:25px;background-color:rgba(0,0,0,0.4);color:white;width:50px;height:50px;text-align:center;border-radius:50%;cursor:pointer;opacity:0.7;transition:opacity .3s;text-decoration:none;line-height:50px;z-index:1000;}.back-to-top:hover{opacity:1;}</style>""", unsafe_allow_html=True)
            ...
            if items_to_display is not None:
                if not items_to_display: st.info("This folder is empty.")
                    ...
                    col_widths, headers = [0.8, 4, 
1, 1, 1.5, 1.5, 2], ["", "Name", "Type", "Size", "Modified", "Owner", "Actions"]
          ...
          row_cols = st.columns(col_widths); is_folder = item['is_folder_sort'] == 1;
nav_id = item.get('shortcutDetails', {}).get('targetId') or item['id']
                        if is_folder:
                      ...
                      with row_cols[1]:
                       ...
                       new_name = st.text_input("New Name", value=item['name'], label_visibility="collapsed");
form_cols = st.columns(2)
    ...
    st.session_state.item_to_rename = None; st.rerun()
                                    if form_cols[1].form_submit_button("❌", use_container_width=True): st.session_state.item_to_rename = None;
st.rerun()
                            else: prefix = "🤝 " if not item.get('is_owned_by_me', True) else "";
st.write(f"{prefix}{get_file_icon(item)} {item['name']}")
                        row_cols[2].write("Folder" if is_folder else "File");
row_cols[3].write(f"{int(item.get('size', 0)) / (1024*1024):.2f} MB" if not is_folder and item.get('size') else ""); row_cols[4].write(pd.to_datetime(item['modifiedTime']).strftime('%y-%m-%d %H:%M'));
row_cols[5].write(item.get('effective_owner_name', 'N/A'))
                        with row_cols[6]:
                            action_cols = st.columns(3)
                            if action_cols[0].button("✏️", key=f"rename_btn_{item['id']}", help="Rename"): st.session_state.item_to_rename = item['id'];
st.rerun()
                            if action_cols[1].button("🗑️", key=f"delete_btn_{item['id']}", help="Delete"): st.session_state.item_to_delete = item;
st.rerun()
                            if action_cols[2].button("📋", key=f"copy_btn_{item['id']}", help="Copy to my Drive"): st.session_state.update(link_to_copy=item.get('webViewLink'), auto_fetch_on_load=True, page="Cloud Copy");
st.rerun()
                        if st.session_state.item_to_delete and st.session_state.item_to_delete['id'] == item['id']:
                            st.warning(f"Are you sure you want to delete **{st.session_state.item_to_delete['name']}**?");
...
st.rerun()
                            if del_cols[1].button("❌ Cancel", key=f"cancel_del_{item['id']}"): st.session_state.item_to_delete = None;
st.rerun()
        ...
        def fetch_source_details(service, link):
            st.session_state.fetched_file_details, st.session_state.folder_contents_df, st.session_state.copied_files_df, st.session_state.dest_id = None, None, None, None;
st.session_state.skipped_files_df = None
    ...
    if contents: st.session_state.folder_contents_df = create_standard_dataframe(contents)
     ...
     if st.session_state.pop('auto_fetch_on_load', False): link_to_process = st.session_state.pop('link_to_copy', ""); st.session_state.drive_link_input = link_to_process;
fetch_source_details(service, link_to_process)
...
if st.button("Fetch Details", key="fetch_details_button"): fetch_source_details(service, st.session_state.drive_link_input)
        if st.session_state.fetched_file_details:
            details = st.session_state.fetched_file_details;
st.markdown("---"); st.subheader(f"Source Details: {get_file_icon(details)} {details.get('name')}")
 ...
 st.markdown("##### File Contents"); c1, c2, c3 = st.columns(3)
                with c1: select_all = st.checkbox("Select/Deselect All", value=True, key="cc_select_all");
st.caption("If none selected, ALL files will be copied.")
                with c2: show_raw = st.checkbox("Show Raw Data", value=False)
                with c3: excel_data, _ = generate_excel_report({'File List': st.session_state.folder_contents_df});
st.download_button("📥 Download List as Excel", excel_data, f"{details.get('name', 'file_list')}.xlsx")
                df = st.session_state.folder_contents_df.copy();
df['Select'] = select_all; visible_columns = ['Select', 'Name', 'Type', 'Size (MB)', 'Modified', 'Owner', 'Link', 'Path'];
column_config = { "Link": st.column_config.LinkColumn("File Link", display_text="LINK"), "Size (MB)": st.column_config.NumberColumn(format="%.2f MB") }
                if not show_raw:
...
st.rerun()
      ...
      st.session_state.copied_files_df = None; st.session_state.skipped_files_df = None; dest_id = folder_ids[folder_names.index(selected_folder_name)];
final_dest_name = new_folder_name if new_folder_name else selected_folder_name
                        if new_folder_name:
                            with st.spinner(f"Creating folder '{new_folder_name}'..."): new_folder = service.files().create(body={'name': new_folder_name, 'mimeType': 'application/vnd.google-apps.folder', 'parents': [dest_id]}, fields='id').execute();
dest_id = new_folder['id']
                        st.session_state.dest_id = dest_id;
copied_files_list, skipped_files_list = [], []; progress_bar = st.progress(0, text="Starting copy process...")
                        total_size_copied = 0
                        for i, row in enumerate(selected_files.itertuples(name="Pandas")):
                            progress_text = f"Processing ({i+1}/{len(selected_files)}): {row.Name}";
progress_bar.progress((i + 1) / len(selected_files), text=progress_text)
                            try: caps_dict = ast.literal_eval(row.capabilities) if isinstance(row.capabilities, str) else row.capabilities
                            except: caps_dict = {}
                            
if not caps_dict.get('canCopy', True): skipped_files_list.append({'Name': row.Name, 'Reason': 'Copying disabled by owner'});
continue
                            file_meta = {'name': row.Name.replace('📁 ', '').replace('📄 ', ''), 'parents': [dest_id]}
                            try:
                                copied_file = service.files().copy(fileId=row.id, 
body=file_meta, supportsAllDrives=True, fields='id, name, webViewLink, size, mimeType').execute()
       ...
       copied_files_list.append({'Name': copied_file['name'], 'Type': row.Type, 'Size (MB)': float(f"{size_bytes / (1024*1024):.2f}"),'Modified': row.Modified, 'Owner': storage['user_name'], 'Link': copied_file.get('webViewLink', '#'), 'Path': os.path.join(final_dest_name, copied_file['name'])})
                            except HttpError as e: skipped_files_list.append({'Name': row.Name, 'Reason': f"Error: {e.reason}"});
continue
                        st.session_state.copied_files_df = pd.DataFrame(copied_files_list) if copied_files_list else pd.DataFrame();
st.session_state.skipped_files_df = pd.DataFrame(skipped_files_list) if skipped_files_list else pd.DataFrame()
                        end_time = time.time()
...
st.rerun()
      ...
      st.markdown("---"); st.subheader("Process Results"); visible_columns = ['Name', 'Type', 'Size (MB)', 'Modified', 'Owner', 'Link', 'Path'];
column_config = { "Link": st.column_config.LinkColumn("File Link", display_text="LINK"), "Size (MB)": st.column_config.NumberColumn(format="%.2f MB"), "Path": st.column_config.TextColumn("Destination Path") }
            if st.session_state.copied_files_df is not None and not st.session_state.copied_files_df.empty: st.write("#### ✅ Copied Files");
df_results = st.session_state.copied_files_df; display_cols = [col for col in visible_columns if col in df_results.columns];
st.dataframe(df_results, column_order=display_cols, column_config=column_config, hide_index=True, use_container_width=True)
            if st.session_state.skipped_files_df is not None and not st.session_state.skipped_files_df.empty: st.write("#### ⚠️ Skipped Files");
st.dataframe(st.session_state.skipped_files_df, hide_index=True, use_container_width=True)
            report_dfs = {'Copied_Files': st.session_state.copied_files_df, 'Skipped_Files': st.session_state.skipped_files_df};
excel_data, _ = generate_excel_report(report_dfs, "copy_report.xlsx"); st.download_button("📥 Download Full Report as Excel", excel_data, "copy_report.xlsx")
        ...
        **How to use it:**
        ...
        2.  **Review the analysis:** The app will suggest a 
common promotional tag to remove from filenames.
        ...
        if st.button("Fetch & Analyze", key="cleaner_fetch"):
            file_id 
= extract_file_id_from_link(st.session_state.cleaner_link)
            if file_id:
                root, items = get_owner_and_all_items_recursive(service, file_id)
                if root: st.session_state.cleaner_root_details = root;
st.session_state.cleaner_all_items = items; st.session_state.cleaner_state = 'analyzed'; st.session_state.cleaner_success_log = None; st.session_state.cleaner_skipped_log = None
        ...
        if st.session_state.cleaner_state in ['analyzed', 'finished']:
            root = st.session_state.cleaner_root_details;
items = st.session_state.cleaner_all_items; capabilities = root.get('capabilities', {}); can_edit_directly = capabilities.get('canDelete', False) and capabilities.get('canRename', False);
all_content = [root] + items if root.get('mimeType') == 'application/vnd.google-apps.folder' else [root]
            st.markdown("---");
st.subheader(f"{get_file_icon(root)} {root.get('name')}")
           ...
           df_full_list = create_standard_dataframe(all_content)
                if not df_full_list.empty: excel_data, excel_filename = generate_excel_report({'File_List': df_full_list}, f"{root.get('name', 'drive_content')}_full_list.xlsx");
st.download_button("📥 Download Full List as Excel", excel_data, excel_filename, "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
            st.markdown("---");
st.subheader("Analysis and Cleaning Actions"); suggested_tag, suggested_promo_files = analyze_content(all_content)
...
col1, col2 = st.columns(2)
            ...
            st.markdown("**2. Select Files to Process**");
select_all = st.checkbox("Select/Deselect All", value=True, key="cleaner_select_all"); st.caption("Note: If no files are selected, ALL files will be processed.");
st.caption("Click on a cell in the 'Action' column to change it.")
...
column_config = { "Link": st.column_config.LinkColumn("File Link", display_text="LINK"), "Size (MB)": st.column_config.NumberColumn(format="%.2f MB"), "Action": st.column_config.SelectboxColumn("Action", options=["Copy", "Exclude"] if not can_edit_directly else ["Rename", "Delete", "Keep"], required=True), "Name": st.column_config.TextColumn("File Name", disabled=True), }
            ...
            with st.form("submission_form"):
                st.markdown("**3. Choose Destination (for copying shared content)**");
user_folders = get_user_folders(service); folder_names, folder_ids = ["My Drive (Root)"] + [f['name'] for f in user_folders], ["root"] + [f['id'] for f in user_folders];
dest_col1, dest_col2 = st.columns(2)
                ...
                button_text = "🚀 Start Cleaning Process" if can_edit_directly else "🚀 Start Copying and Cleaning Process";
submitted = st.form_submit_button(button_text, type="primary")
                if submitted:
                    ...
                    if not edited_df.empty:
                        ...
                        log_entries = [];
final_dest_id = dest_folder_id; new_root_folder_name = ""
                        start_time = time.time()
                        total_size_copied = 0
                        with st.spinner("Processing files... Please wait."):
               
             if not can_edit_directly:
                                new_root_folder_name = new_folder_name if new_folder_name else root.get('name');
st.session_state.cleaner_dest_folder_name = new_root_folder_name; st.text(f"Creating new root folder: '{new_root_folder_name}'"); new_folder_meta = {'name': new_root_folder_name, 'mimeType': 'application/vnd.google-apps.folder', 'parents': [dest_folder_id]};
new_folder = service.files().create(body=new_folder_meta, fields='id', supportsAllDrives=True).execute(); final_dest_id = new_folder.get('id')
 ...
 progress_bar.progress((i + 1) / len(actions_to_perform), text=f"Processing: {row.Name}"); log_entry = {'Status': 'Skipped', 'Name': row.Name, 'New Name': row.New_Name, 'Path': row.Path, 'Size (MB)': row._asdict().get('Size (MB)'), 'Link': 'N/A', 'Owner': row.Owner, 'Modified': row.Modified, 'Type': row.Type}
                                if can_edit_directly:
                                    if 
row.Action == 'Delete':
                                        try: service.files().delete(fileId=row.id, supportsAllDrives=True).execute();
log_entry.update({'Status': 'Deleted', 'New Name': 'N/A', 'Size (MB)': 'N/A'})
                                        except HttpError as e: log_entry.update({'Status': f'Error Deleting: {e.reason}'})
                                    elif row.Action == 'Rename' and row.Name != row.New_Name:
   
                                     try: updated_file = service.files().update(fileId=row.id, body={'name': row.New_Name}, supportsAllDrives=True, fields='webViewLink, size').execute();
log_entry.update({'Status': 'Renamed', 'Link': updated_file.get('webViewLink'), 'Size (MB)': float(f"{int(updated_file.get('size', 0)) / (1024*1024):.2f}") if updated_file.get('size') else 'N/A', 'Path': row.Path})
         ...
         file_meta = {'name': row.New_Name, 'parents': [final_dest_id]};
copied_file = service.files().copy(fileId=row.id, body=file_meta, supportsAllDrives=True, fields='id, name, webViewLink, size').execute()
                                                size_bytes = int(copied_file.get('size', 0))
       ...
       total_size_copied += size_bytes
            ...
            log_entry.update({'Status': 'Copied to Drive', 'New Name': copied_file['name'], 'Path': dest_path, 'Size (MB)': float(f"{size_bytes / (1024*1024):.2f}"), 'Link': copied_file.get('webViewLink', '#'), 'Owner': storage['user_name']})
              ...
              log_entries.append(log_entry)
                        if log_entries: df_log = pd.DataFrame(log_entries);
st.session_state.cleaner_success_log = df_log[df_log['Status'].isin(['Renamed', 'Deleted', 'Copied to Drive'])]; st.session_state.cleaner_skipped_log = df_log[~df_log['Status'].isin(['Renamed', 'Deleted', 'Copied to Drive'])]
                        else: st.session_state.cleaner_success_log = pd.DataFrame();
st.session_state.cleaner_skipped_log = pd.DataFrame()
                        end_time = time.time()
                        duration = end_time - start_time
                        rate = (total_size_copied / duration) / (1024*1024) if duration > 0 else 0
...
st.session_state.cleaner_state = 'finished'; st.rerun()
        if st.session_state.cleaner_state == 'finished':
            # The summary bar will be displayed by the global logic at the top of the run_main_app function.
            ...
            results_config = {"Link": st.column_config.LinkColumn("File Link", display_text="LINK"),"Size (MB)": st.column_config.NumberColumn(format="%.2f MB"),"Path": 
st.column_config.TextColumn("Destination Path"),"Name": st.column_config.TextColumn("File Name")}
            if st.session_state.cleaner_success_log is not None and not st.session_state.cleaner_success_log.empty: st.write("#### Successful Actions");
df_success = st.session_state.cleaner_success_log; st.dataframe(df_success, use_container_width=True, hide_index=True, column_config=results_config)
            if st.session_state.cleaner_skipped_log is not None and not st.session_state.cleaner_skipped_log.empty: st.write("#### ⚠️ Skipped Files & Errors");
df_skipped = st.session_state.cleaner_skipped_log; st.dataframe(df_skipped, use_container_width=True, hide_index=True)
            report_dfs = {'Successful_Actions': st.session_state.cleaner_success_log, 'Skipped_and_Errors': st.session_state.cleaner_skipped_log};
excel_data, _ = generate_excel_report(report_dfs, "cleaning_report.xlsx"); st.download_button("📥 Download Full Report", excel_data, "cleaning_report.xlsx")
...
if service:
    ...
    if user_info:
        ...
        st.error("Could not retrieve user information from 
Google. Please try logging in again.")
